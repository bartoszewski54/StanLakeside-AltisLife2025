...
if (_selectableCarts isEqualTo []) exitWith {
	["Nie masz zapisanych koszyków"] call fini_fnc_noty;
};
...
[(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), _selectableCarts, 
	...
	if (_index isEqualTo -1) exitWith { ["Nie wybrano koszyka do załadowania!"] call fini_fnc_noty; };
	...
	[format ["Loaded Cart '%1'", _selectedCartName]] call fini_fnc_noty;
