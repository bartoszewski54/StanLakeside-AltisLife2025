...
if (isNull _display) exitWith { systemChat "Błąd koszyka" };
...
if (isNull _itemList || { isNull _cartList } || { isNull _textList }) exitWith { systemChat "Błąd menu koszyka" };

private _curItem = lbCurSel _itemList;

if (_curItem isEqualTo -1) exitWith {
	["Musisz wybrać przedmiot, aby dodać go do koszyka!"] call fini_fnc_noty;
};
