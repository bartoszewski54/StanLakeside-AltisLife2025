...
private _reqItems = getArray (_zone >> "reqItems");
{
	if ([_x] call ULP_fnc_hasItem isEqualTo -1) exitWith {
		[format["Ta strefa wymaga: <t color='#B92DE0'>%1</t>", _reqItems]] call fini_fnc_noty;
...
private _reqLicenses = getArray (_zone >> "reqLicenses");
{
	if !([_x] call ULP_fnc_hasLicense) exitWith {
		[format["Potrzebujesz licencji: <t color='#B92DE0'>%1</t>!", _reqLicenses]] call fini_fnc_noty;
		false breakOut "fn_gather";
	};
...
if (_total <= 0) exitWith {
	["Masz pełny ekwipunek"] call fini_fnc_noty;
	false
};
...
[format["%1 %2(s)", _title, getText(_item >> "displayName")], _time * _total, [_item, _total, _profession, _leveling], { true }, {
	...
	[format["Zebrałeś <t color='#B92DE0'>%1 %2(s)</t>.", _total, getText(_item >> "displayName")]] call fini_fnc_noty;
	if ((random 100) <= 5) then {
        [5, "Zbieranie"] call ULP_fnc_addXP;
        ["Otrzymałeś <t color='#B92DE0'>5 XP</t> za zbieranie!"] call fini_fnc_noty;
    };
