...
if !([] call ULP_fnc_isGroupOwner) exitWith {
	["Aby usunąć grupę, musisz być jej właścicielem!"] call fini_fnc_noty;
	false
};
...
ULP_DisbandEvent = ["GroupDisbanded", {
	...
	ULP_HouseSellEvent = ["HouseSold", {
		...
		if (_success) then { [_money, true, "Sprzedanie domu"] call ULP_fnc_addMoney; };

		ULP_HousesToSell = ULP_HousesToSell - 1;
		if (ULP_HousesToSell < 1) then {
			["All group dependant housing has been sold!"] call fini_fnc_noty;
...
[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), "Potwierdzenie", ["Usuń Grupę", "Anuluj"],
	format ["Jesteś pewny że chcesz <t color='#B92DE0'>USUNĄĆ %1</t>...", [_group] call ULP_fnc_getGroupName], [_group],
	{	
		_this params [ "_group" ];
