...
if (time < (player getVariable ["pnc_cd", 0])) exitWith {
	["Zostałeś tymczasowo zablokowany z używania komputera..."] call fini_fnc_noty;
};

if ([player] call ULP_fnc_isRestrained) exitWith {
	["Nie można uzyskać dostępu do komputera, gdy jest się zakutym!"] call fini_fnc_noty;
};

if (createDialog "DialogPNC") then {
	private _display = findDisplay 4700;

	(_display displayCtrl 4701) ctrlSetStructuredText parseText format ["Komputer Policyjny <t align='right'>Poziom zagrożenia: %1</t>",
	...
	[_display, _login, _main, _userInput, _passInput] spawn {
		...
		if !(_loggedIn) exitWith {
			["Nie udało ci się włamać do komputera policyjnego..."] call fini_fnc_noty;
