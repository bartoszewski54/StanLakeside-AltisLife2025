...
if (isNull _unit || { _crime isEqualTo "" } || { _crimeId isEqualTo -1 }) exitWith {
	["Musisz wybrać przestępstwo do usunięcia..."] call fini_fnc_noty;
};

[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), "Potwierdzenie", ["Usuń", "Anuluj"],
	format ["Czy na pewno chcesz usunąć %1 z nakazu %2?...", getText (missionConfigFile >> "CfgWarrants" >> _act >> _crime >> "displayName"), name _unit], [_unit, _act, _crime, _crimeId, _display],
	{	
		...
		[format ["Usunięto <t color='#B92DE0'>%1</t> przestępstw z nakazu <t color='#B92DE0'>%2</t> ", getText (missionConfigFile >> "CfgWarrants" >> _act >> _crime >> "displayName"), name _unit]] call fini_fnc_noty;
	}, {}, false
