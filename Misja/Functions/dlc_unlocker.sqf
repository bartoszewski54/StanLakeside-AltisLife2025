...
DLC_Functions_AddAction = {
	...
	DLC_AddAction_1 = _vehicle addAction ["<img image='IgiLoad/images/load.paa' size='2' align='left'/><t color='#FF0000'><t/> " + _displayVehicle + " <t color='#FF0000'>Wejdz do pojazdu -- DLC</t>",{player moveInDriver (_this select 0)},[],1,false,true,"","_this distance _target < 8"];
...
DLC_Functions_RemoveAction = {
    _vehicle = _this select 0;
    _action = _vehicle getVariable ["DLC_Var_AddAction_Number", -1]; // Jeśli zmienna nie istnieje, zwróci -1

    if (_action != -1) then {
        _vehicle removeAction _action;
        _vehicle setVariable ["DLC_Var_AddAction_Active", false];
        _vehicle setVariable ["DLC_Var_AddAction_Number", nil]; // Usunięcie zmiennej, by uniknąć kolejnych błędów
    };
};
