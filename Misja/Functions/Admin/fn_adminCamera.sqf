...
switch (_mode) do {
	...
	case "action": {
		...
		if (time < (player getVariable ["admin_action_cooldown", 0])) exitWith {
			["Odczekaj chwile...", "AdminMode"] call fini_fnc_noty;
			false
		};

		private _focus = ["GetCameraTarget"] call ULP_fnc_adminCamera;
		if (isNull _focus) exitWith { ["Musisz kogoś wybrać...", "AdminMode"] call fini_fnc_noty; };

		player setVariable ["admin_action_cooldown", time + 2];

		switch (_type) do {
			case "Eject": {
				if !([_focus] call ULP_fnc_ejectVehicleUnit) exitWith { [format ["<t color='#B92DE0'>%1</t> nie jest w pojeździe...", name _focus]] call fini_fnc_noty; };
			...
			if (_focus isEqualTo player) exitWith { ["Nie wykonasz tego na sobie..."] call fini_fnc_noty; };
			if !(isNull (objectParent player)) exitWith { ["Wyjdź z pojazdu, aby to wykonać.."] call fini_fnc_noty; };

			case "To": {	
				player setPos (getPos _focus);
				[format ["Teleportowałeś się do <t color='#B92DE0'>%1</t>", name _focus]] call fini_fnc_noty;
			...
			case "Here": {
				...
				[format ["Teleportowałeś <t color='#B92DE0'>%1</t> do siebie", name _focus]] call fini_fnc_noty;
			...
			case "Vehicle": {
				if (isNull (objectParent _focus)) exitWith { [format ["<t color='#B92DE0'>%1</t> nie jest w pojeździe...", name _focus]] call fini_fnc_noty; };
								
				private _vehicle = (vehicle _focus);

				if (count (crew _vehicle) >= ((([typeOf _vehicle] call ULP_fnc_vehicleCfg) param [6, 0]) + 1)) exitWith {
					[format ["<t color='#B92DE0'>%1</t> ma pełny pojazd...", name _focus]] call fini_fnc_noty;
				};
	...
	case "ViewToolboxChanged": {
		...
		if (isNull _focus && { _index isEqualTo 1 }) exitWith {
			["Wybierz target do spectowania..."] call fini_fnc_noty;
			_ctrl lbSetCurSel 0;
		};
