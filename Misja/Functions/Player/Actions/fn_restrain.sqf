...
if !(local _target) exitWith {
	if ([player, ["Police"]] call ULP_fnc_isFaction || { ["Ziptie", 1, _restrain] call ULP_fnc_handleItem } || { ["cuffCourier"] call ULP_fnc_hasPerk } || { [_detainer] call ULP_fnc_onDuty }) then {
		...
		["Potrzebujesz opasek by kogoś zakuć"] call fini_fnc_noty;
	};
};
...
if (_restrain) then {
	...
	[format ["Zostałeś związany przez <t color='#B92DE0'>%1</t>.", name _detainer]] call fini_fnc_noty;
	...
	[format ["Zostałeś rozwiązany przez %1.", ([format ["by <t color='#B92DE0'>%1</t>", name _detainer], ""] select (isNull _detainer))]] call fini_fnc_noty;
};