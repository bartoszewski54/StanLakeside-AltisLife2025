...
if ([] call ULP_fnc_donatorLevel > 0) then { _xp = _xp * getNumber(missionConfigFile >> "CfgSettings" >> "DonationRewards" >> "xpIncrease"); };
...
if !(ULP_Level isEqualTo _previousLevel) then {
	["Zdobyłeś doświadczenie!"] call fini_fnc_noty;
} else {
	[format["Zdobyłeś <t color='#B92DE0'>%1</t> XP za %2.", [_xp] call ULP_fnc_numberText, _reason]] call fini_fnc_noty;
};
