...
if (_action isEqualTo 0) then {
	...
	if ([format["Obracanie %1", _name], _time, [_vehicle], { !(isNull (_this select 0)) && { alive (_this select 0) } && { (player distance (_this select 0)) <= 5 } }, {
		...
		["Obróciłeś pojazd..."] call fini_fnc_noty;
