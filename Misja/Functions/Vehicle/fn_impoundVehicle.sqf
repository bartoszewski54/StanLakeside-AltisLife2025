...
if !([player, ["Police", "Hato"]] call ULP_fnc_isFaction) exitWith {
	["Tylko policja i HATO mogą konfiskować pojazdy"] call fini_fnc_noty;
};

if !((crew _vehicle) isEqualTo []) exitWith {
	["Nikt nie może przebywać w pojeździe podczas jego konfiskaty!"] call fini_fnc_noty;
};

if ((_vehicle getVariable ["vehicle_id", -1]) < 0) exitWith {
	["Wypożyczone pojazdy nie mogą być konfiskowane!"] call fini_fnc_noty;
};
...
if ((_vehicle getVariable ["vehicle_id", -1]) < 0) exitWith {
	["Wypożyczone pojazdy nie mogą zostać skonfiskowane!"] call fini_fnc_noty;
};
...
[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), getArray (missionConfigFile >> "CfgSettings" >> "Police" >> "impoundFee"), [_vehicle, _name, _time],
	{
		...
		if !([format["Konfiskowanie %1", _name], _time, [_vehicle, _name, _fee], {
			...
			if (isNull _vehicle || { !((crew _vehicle) isEqualTo []) }) exitWith {
				[format["Nie udało się skonfiskować tego pojazdu, ponieważ albo ktoś w nim był, albo został już usunięty!"]] call fini_fnc_noty;
			};
			...
			[format["Zażądano skonfiskowania <t color='#B92DE0'>%1</t> z ceną wykupu <t color='#B92DE0'>%2%3</t>.", _name, "$", [_fee] call ULP_fnc_numberText]] call fini_fnc_noty;
			["VehicleStored", { ["Pojazd odholowany na parking policyjny."] call fini_fnc_noty; }, true] call ULP_fnc_addEventHandler;
			...
			["Nie można skonfiskować pojazdu podczas wykonywania innej czynności!"] call fini_fnc_noty;
		};
