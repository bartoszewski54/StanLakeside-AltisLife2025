...
//Wyrzuc itemki i kase

private _deathMessage = format["<t align='center' size='2'>NIEPRZYTOMNY</t>"];
ULP_CarryWeight = 0;
...
if (!isNull _killer && { isPlayer _killer } && { !(_killer isEqualTo _unit) }) then {
	...
	_deathMessage = format["<t align='center' size='2'>%1 poważnie cię zranił</t>", _killerName];
...
//[getPos _unit] call ULP_fnc_dropMoney; //tworzy pakunek z kasą | 1.2.6 - nieuzywane
[getPos _unit, getDir _unit] call ULP_fnc_createBodyBag; // tworzy skrzynke z itemami
ULP_Inventory = createHashMap; //czysci eq
[true] call ULP_fnc_saveGear; //zapisuje eq zeby debile nie robili alt f4
