...
with missionNamespace do {
	...
	_fieldButton ctrlSetText "Stanlakeside.pl"; //ustawić nazwe serwera
	...
	if !(["Disconnect", false] call ULP_fnc_checkPower) then {
		...
		[[_abortButton, _abortDelay], {
			...
			if (time < _abortDelay) then {
				_abortButton ctrlSetText format["WYJDŹ (%1)", [_abortDelay - time, "SS.MS"] call BIS_fnc_secondsToString];
			} else {
				_abortButton ctrlSetText "WYJDŹ";
