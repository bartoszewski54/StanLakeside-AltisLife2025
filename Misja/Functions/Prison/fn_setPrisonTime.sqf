...
if !([player] call ULP_fnc_canImprison || { [_target] call ULP_fnc_canImprisoned }) exitWith {
	["Nie możesz stąd wysłać nikogo do więzienia!"] call fini_fnc_noty;
};
...
[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), [1, _max], [_target, _prison, _max],
	{
		...
		if (_duraction > _max) exitWith {
			[format ["Można skazać kogoś na maksymalnie <t color='#B92DE0'>%1</t>", [_max, "MM:SS"] call BIS_fnc_secondsToString]] call fini_fnc_noty;
		};
		...
		[format["Skazałeś <t color='#B92DE0'>%1</t> na <t color='#B92DE0'>%2</t>", name _target, [_duraction, "MM:SS"] call BIS_fnc_secondsToString]] call fini_fnc_noty;
