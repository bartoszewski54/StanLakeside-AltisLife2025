...
if ([] call ULP_fnc_donatorLevel > 0) then { _buyPrice = _buyPrice / getNumber(missionConfigFile >> "CfgSettings" >> "DonationRewards" >> "vehicleDecrease"); };
...
if (isClass _textureCfg) then {
	if !([configName _missionCfg, _texture] call ULP_fnc_isTextureUnlocked) then {
		[format ["Tekstura <t color='#B92DE0'>%1</t> jest obecnie zablokowana; musisz ją odblokować, zanim będziesz mógł jej użyć!", getText (_textureCfg >> "displayName")]] call fini_fnc_noty;
...
[
	...
	format ["<t color='#B92DE0'>%1</t> wymaga DLC, którego nie posiadasz. Czy", _name],
	[_display, _missionCfg, _buyPrice, _texture],
	{
		...
		if (isNil "_spawn") exitWith {
			["Brak wolnych miejsc do spawnu pojazdu."] call fini_fnc_noty;
		};

		private _faction = [player] call ULP_fnc_getFaction;


		if ([_faction, "vehicles"] call ULP_fnc_factionFree || { [_buyPrice, false, format ["Kupno %1", _name]] call ULP_fnc_removeMoney }) exitWith {
			...
			if ([_faction, "vehicles"] call ULP_fnc_factionPresistant) then {
				["VehicleBought", {
					...
					if (_limitReached) exitWith {
						...
						[_price, false, "Zwrot za zakup"] call ULP_fnc_addMoney;
						[([format ["Twój zakup nie mógł zostać zrealizowany, ponieważ osiągnąłeś maksymalny limit przechowywania pojazdów %1/<t color='#B92DE0'>%2</t>.", _name, [_limit] call ULP_fnc_numberText],
							format ["Zostało Ci zwrócone <t color='#B92DE0'>%1%2</t> za %3, ponieważ osiągnąłeś maksymalny limit przechowywania dla <t color='#B92DE0'>%4</t>...", "$", [_price] call ULP_fnc_numberText, _name, [_limit] call ULP_fnc_numberText]] select (_price > 0))] call fini_fnc_noty;
					};
		...
		[format["Nie stać Cie na ten pojazd, potrzebujesz $%1.", [_buyPrice] call ULP_fnc_numberText]] call fini_fnc_noty;
	}, {}, false, _hasDlc
