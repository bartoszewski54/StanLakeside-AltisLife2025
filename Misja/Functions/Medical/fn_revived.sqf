...
if ([_medic] call ULP_fnc_onDuty) then {
	["Zostałeś ożywiony przez administratora..."] call fini_fnc_noty;
} else {
	[format ["Zostałeś ożywiony przez <t color='#B92DE0'>%1</t>", [_medic, true] call ULP_fnc_getName]] call fini_fnc_noty;
};

if !(player isEqualTo _medic) then {
	//["Revived", [name player, [[_medic, true] call ULP_fnc_getName, "Administratora"] select ([_medic] call ULP_fnc_onDuty)]] remoteExecCall ["ULP_fnc_chatMessage", RCLIENT];
