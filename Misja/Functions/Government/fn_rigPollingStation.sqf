...
if (_groupId isEqualTo -1 || { !(["PollRigging", _group] call ULP_fnc_hasGroupPerk) }) exitWith {
	["Musisz należeć do grupy, która ma perk do fałszowania lokali wyborczych."] call fini_fnc_noty;
};
...
if (_isRigged && { _riggedGroupId isEqualTo _groupId }) exitWith {
	["Ten lokal wyborczy został już sfałszowany przez waszą grupę."] call fini_fnc_noty;
};
...
["Fałszowanie głosów...", _time, [_pollingStation, _group, _groupId],
	{ !(((_this select 0) getVariable ["rigged_group", grpNull]) isEqualTo (_this select 1)) && { (player distance (_this select 0)) <= 5 } }, 
	{
		...
		["PollingStationRigged", {
			_this params [
				["_message", "Stacja wyborcza pomyślnie sfałszowana", [""]]
			];
