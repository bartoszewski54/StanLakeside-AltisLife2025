...
if !(_type in ULP_Missions) exitWith {
	[getText (_typeCfg >> "Messages" >> "onFinishWithoutMission")] call fini_fnc_noty;
};
...
if ((player distance (taskDestination _task)) > 10) exitWith {
	["Nie jesteś wystarczająco blisko, by wykonać to zadanie!"] call fini_fnc_noty;
};
...
if !(_msg isEqualTo "") then {
	[format [getText (_typeCfg >> "Messages" >> "onFinished"), [_reward] call ULP_fnc_numberText]] call fini_fnc_noty;
};
