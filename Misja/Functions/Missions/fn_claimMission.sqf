...
if !([player, getArray (_typeCfg >> "factions")] call ULP_fnc_isFaction) exitWith {
	["Twoja frakcja nie może wykonać tej misji..."] call fini_fnc_noty;
	false
};

if !(call compile getText(_typeCfg >> "conditions")) exitWith {
	["Nie spełniasz wymagań do wykonania tej misji..."] call fini_fnc_noty;
	false
};

if (_type in ULP_Missions) exitWith {
	[getText (_typeCfg >> "Messages" >> "onAlreadyHas")] call fini_fnc_noty;
	false
};
...
if (isNil "_location" || { _location isEqualTo [] }) exitWith {
	[getText (_typeCfg >> "Messages" >> "onNoLocations")] call fini_fnc_noty;
	false
};
...
[format [getText (_typeCfg >> "Messages" >> "onAssigned"), _locName]] call fini_fnc_noty;
