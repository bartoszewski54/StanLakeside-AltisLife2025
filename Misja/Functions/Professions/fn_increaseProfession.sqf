...
if ([] call ULP_fnc_donatorLevel > 0) then { _chance = (_chance * getNumber(missionConfigFile >> "CfgSettings" >> "DonationRewards" >> "profChanceIncrease")) min 100; };
...
if (_previousLevel < _level) then {
	["Twoje doświadczenie w profesji <t color='#B92DE0'>wzrosło</t>!"] call fini_fnc_noty;
} else {
	["<t color='#B92DE0'>Ulepszyłeś</t> swoją profesję!"] call fini_fnc_noty;
};
