...
if !([format["Leczenie %1", name _target], _time, [_target, _hasMedkit], { [(_this select 0)] call ULP_fnc_canHeal }, {
	...
	[format ["Uleczyłeś <t color='#B92DE0'>%1</t>!", name _target]] call fini_fnc_noty;
}, {}, ["GRAB", (["PRONE", "CROUCH"] select ((stance player) isEqualTo "STAND"))]] call ULP_UI_fnc_startProgress) exitWith {
	["Nie możesz się leczyć, wykonując jednocześnie inną czynność..."] call fini_fnc_noty;
	false
};
