...
if ([_item, _data, false, _return] call ULP_fnc_handleItem) exitWith {
	...
	private _hint = format ["%1 dał Ci %2 %3", [_unit, true] call ULP_fnc_getName, [_count] call ULP_fnc_numberText, _displayName];

	if (_return) then {
		_hint = format ["%1 zwrócił Ci %2 %3s ponieważ nie ma na nie miejsca!", [_unit, true] call ULP_fnc_getName, [_count] call ULP_fnc_numberText, _displayName];
	};

	[_hint] call fini_fnc_noty;
	true
};
