...
if ((lbCurSel 1500) isEqualTo -1) exitWith {hint "Nie wybrano miejsca docelowego";};
...
private _multiplyer = getNumber(missionConfigFile >> "CfgSettings" >> "Toxic_Bus_Config" >> _side >> "multipler");
private _tripTotal = 15000;

private _busStops = getArray(missionConfigFile >> "CfgSettings" >> "Toxic_Bus_Config" >> _side >> "stops");
private _payment = getText(missionConfigFile >> "CfgSettings" >> "Toxic_Bus_Config" >> "payment");
private _type = getNumber(missionConfigFile >> "CfgSettings" >> "Toxic_Bus_Config" >> "bus_3d");
...
if (call life_donator > 0) then {
      _result = [ format ["Czy na pewno chcesz podróżować Do %1?", _busDepotName], "Bus", "Tak", "Nie"] call BIS_fnc_guiMessage;
} else {
      _result = [ format ["Czy na pewno chcesz podróżować Do %1? Będzie cię kosztować $%2", _busDepotName, _tripTotal], "Bus", "Tak", "Nie"] call BIS_fnc_guiMessage;
};
if !(_result) exitWith {
...
if (_niemakasy) exitWith {	hint "nie masz pieniedzy";};
