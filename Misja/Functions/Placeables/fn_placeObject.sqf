...
if !(isNull (objectParent player)) exitWith {
	["Musisz znajdować się poza pojazdem..."] call fini_fnc_noty;
	false
};

if !(isNull (missionNamespace getVariable ["ULP_PlaceableObject", objNull])) exitWith {
	["Musisz umieścić obecny obiekt, zanim będziesz mógł umieścić inny..."] call fini_fnc_noty;
	false
};
...
if (([] call ULP_fnc_placedObjects) >= _maxPlaceables) exitWith {
	[format ["Masz %1 aktywnych obiektów, usuń kilka aby umieścić kolejne...", _maxPlaceables]] call fini_fnc_noty;
	false
};
...
[{ !dialog }, [_cfg, _item], {
	...
	_object setDir _rot;
	_object setVariable ["object_owner", [getPlayerUID player, profileName, _item, configName _cfg], true];

	ULP_PlaceableObject = _object;

	ULP_PlaceableObjectAction = player addAction [format ["Postaw %1", getText(_cfg >> "displayName")], {
		...
		if (isNull _object) exitWith {
			["Wystąpił błąd podczas próby umieszczenia tego obiektu..."] call fini_fnc_noty;
		};

		[format ["Stawianie %1", getText(_cfg >> "displayName")], getNumber(_cfg >> "placementTime"),
			...
			[format["Położyłeś %1...", getText(_cfg >> "displayName")]] call fini_fnc_noty;
