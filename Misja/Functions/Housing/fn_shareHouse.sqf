...
if !([_house, player, false] call ULP_fnc_isHouseOwner) exitWith {
	["Only the house owner can change whether the house is shared or not!"] call fini_fnc_noty;
	false
};

// Stop spam...
if (time < (_house getVariable ["building_last_shared", 0])) exitWith {
	["You've changed the shared status of this house recently, please wait..."] call fini_fnc_noty;
	false
};
...
[format ["This house is <t color='#B92DE0'>%1</t> shared with your group.", ["no longer", "now"] select (_shared)]] call fini_fnc_noty;
true