...
if !([_house, player, false] call ULP_fnc_isHouseOwner) exitWith {
	["Only the house owner can change the house name!"] call fini_fnc_noty;
	false
};

// Stop spam...
if (time < (_house getVariable ["building_last_renamed", 0])) exitWith {
	["You've changed the name of this house recently, please wait..."] call fini_fnc_noty;
	false
};
_house setVariable ["building_last_renamed", time + 5];

if (_house getVariable ["selling", false]) exitWith {
	["This house is being sold..."] call fini_fnc_noty;
	false
};
...
[
	...
	{	
		...
		if (_house getVariable ["selling", false]) exitWith {
			["This house is already being sold!"] call fini_fnc_noty;
		};

		if ((count _name) > getNumber (missionConfigFile >> "CfgHousing" >> "nameLength")) exitWith {
			["This name is too long!"] call fini_fnc_noty;
		};

		if ((_house getVariable ["building_name", ""]) isEqualTo _name) exitWith {
			["You can't set the house name to the same as it's current value!"] call fini_fnc_noty;
		};
		...
		[format ["You've renamed this house to <t color='#B92DE0'>%1</t>.", _name]] call fini_fnc_noty;
