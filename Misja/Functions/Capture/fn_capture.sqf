...
if !([] call ULP_fnc_isGroup) exitWith {
	["Musisz być w grupie aby przejąc tą miejscówkę..."] call fini_fnc_noty;
	false
};

if !(call compile getText (_cfg >> "condition")) exitWith {
	["Nie spełniasz wymagań do przejmowania..."] call fini_fnc_noty;
	false
};

private _owner = _location getVariable ["site_owner_id", -1];
if (([] call ULP_fnc_groupId) isEqualTo _owner) exitWith {
	["To twoja miejscówka, po co ją znowu przejmować..."] call fini_fnc_noty;
	false
};
...
([format ["Capturing"], getNumber (_cfg >> "time"), [_location, _cfg], { (player distance (_this select 0)) <= 5 }, {
	...
	// Verify...
	if !([] call ULP_fnc_isGroup) exitWith { ["Musisz być w grupie aby przejąc tą miejscówkę..."] call fini_fnc_noty; };

	_location setVariable ["site_owner_id", ([] call ULP_fnc_groupId), true];
	[format ["Przejęto <t color='#B92DE0'>%1</t>", getText (_cfg >> "displayName")]] call fini_fnc_noty;
