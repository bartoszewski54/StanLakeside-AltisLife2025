...
(_display displayCtrl 23001) ctrlSetStructuredText parseText format["<t align='left'>Market</t><t align='right'>Podatek: %1%2</t>", (["Tax", 5] call ULP_fnc_getLegislation), "%"];
	...
	if !([getNumber (_x >> "Settings" >> "isScripted")] call ULP_fnc_bool) then {
		...
		if !(_itemIcon isEqualTo "") then {
			...
			if ((configName _x) isEqualTo (missionNamespace getVariable ["ULP_SRV_Setting_BuffedItem", ""])) then {
				...
				_list lnbSetTooltip [[_row, 0], "Ten przedmiot tymczasowo wzrósł na wartości..."];
			};
		};
