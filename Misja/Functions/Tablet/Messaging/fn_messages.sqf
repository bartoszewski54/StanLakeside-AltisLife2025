...
switch (lbCurSel _toolbox) do {
	case 1: {
		_sendButton ctrlSetStructuredText parseText "<t align = 'center'>Odpowiedz</t>";
		...
		if ((lbSize _inboxList) > 0) then {
			...
			_inboxList lnbAddRow ["Nie masz żadnych wiadomości...", "", ""];
		};
	...
	default {
		_sendButton ctrlSetStructuredText parseText "<t align = 'center'>Wyślij wiadomość</t>";
		...
		if ((lbSize _contactList) > 0) then {
			...
			_contactList lbAdd "Brak kontaktów...";
		};
