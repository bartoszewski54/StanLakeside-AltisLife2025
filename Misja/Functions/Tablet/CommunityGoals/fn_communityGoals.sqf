...
_display setVariable ["community_goals_received", (["CommunityGoalsReceived", {
...

		{
			...
			if (isClass _goalCfg && { [player, getArray (_goalCfg >> "factions")] call ULP_fnc_isFaction }) then {
				...
				_nameCtrl ctrlSetText format ["%1 (%2 dni do zakończenia)", getText (_goalCfg >> "title"), _daysLeft];
				...
				_descCtrl ctrlSetStructuredText parseText format["Obecna nagroda: <t color='#B92DE0'>%1%2</t><br/>Twoja nagroda: <t color='#B92DE0'>%1%3</t>",
...

				{
					...
					_line ctrlSetTooltip format["Poziom %1", _forEachIndex + 1];
					_line ctrlCommit 0;
				...
				_proTxtCtrl ctrlSetTooltip format ["Twój wkład (%1%2)", (_myProgress / (_progress max 1)) * 100, "%"];
