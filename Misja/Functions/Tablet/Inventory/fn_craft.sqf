...
if !(isNull (objectParent player)) exitWith {
	["Nie można wytwarzać przedmiotów w pojeździe..."] call fini_fnc_noty;
	false;
};

private _cfg = _display getVariable ["craft_cfg", configNull];
if !(isClass _cfg) exitWith {
	["Nie wybrałeś blueprinta..."] call fini_fnc_noty;
	false
};

if ([getNumber (_cfg >> "workbench")] call ULP_fnc_bool && { !([] call ULP_fnc_nearWorkbench) }) exitWith {
	["Ten projekt wymaga stołu warsztatowego do skonstruowania..."] call fini_fnc_noty;
	false
};
...
if (_total < 1) exitWith {
	["Nie posiadasz wszystkich materiałów wymaganych do stworzenia tego planu..."] call fini_fnc_noty;
	false
};
...
[
	(findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), [1, _total], [_cfg, _name],
	{
		...
		[format["Kostruowanie... %1(s)", _name], getNumber (_cfg >> "time") * _amount, [_cfg, _name, _amount], { true }, {
			...
			if ([_item, _amount, false, true] call ULP_fnc_handleItem) then {
				...
				[format ["Stworzyłeś %2 %1(s)...", _name, [_amount] call ULP_fnc_numberText]] call fini_fnc_noty;
