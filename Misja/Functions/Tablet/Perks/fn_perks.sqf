...

{
	...
	if (_state isEqualTo 2) then {
		...
		_coverCtrl ctrlSetTooltip format ["Wymaga %1 %2", _requirement select 0, _requirement select 1];
		...
		if (_maxLevel > 1) then {
			...
			_ctrlLevel ctrlSetTooltip format["%1 XP do następnego poziomu", _requiredXp - _xp];
	...
	if (_state isEqualTo 0) then {
		...
		_button ctrlSetTooltip (["Deaktywuj perk", format ["Cena deaktywacji: %1%2", "$", [_cost] call ULP_fnc_numberText]] joinString "\n");
	} else {
		_button ctrlSetTooltip "Aktywuj perk";
	};
