...

{
	...
	_nameCtrl ctrlSetTooltip (["Odblokowane", "Zablokowane"] select (_state isEqualTo 1));
	_nameCtrl ctrlEnable false;

	private _coverCtrl = _achieveCtrl controlsGroupCtrl 103;
	_coverCtrl ctrlShow !(_state isEqualTo 0);
	_coverCtrl ctrlSetTooltip (["Zablokowany", "Tajemnicze osiągnięcie"] select (_state isEqualTo 2));
