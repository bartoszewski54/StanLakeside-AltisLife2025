...
if (isNull (_screen getVariable ["camera", objNull])) then {
	if !(isPiPEnabled) exitWith { ["Musisz mieć uruchomione PIP (Picture In Picture) w ustawienia graficznych Army!"] call fini_fnc_noty; false breakOut "fn_handleCamera"; };
