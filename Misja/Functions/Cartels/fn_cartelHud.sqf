...
switch (_mode) do {
		...
		if (_isPopup) exitWith {
			...
			[[_cartelObj, _area, (_hud controlsGroupCtrl 103), _ctrlProgress, _ctrlScore, _captureTime], {
				...
				private _status = switch (true) do {
                    case !(isNull (objectParent player)): { "W POJEZDZIE" };
                    case ([player] call ULP_fnc_onDuty): { "ADMINMODE" }; // Ignorowanie admina
                    case ((currentWeapon player) in getArray(missionConfigFile >> "CfgSettings" >> "doesntThreaten")): { "NIE MASZ BRONI" };
                    case (([player] call ULP_fnc_onDuty) || {count ([_area, allPlayers, ["Police", "Civilian"], { !(_this in (units (group player))) }] call ULP_fnc_unitsInZone) > 0}): { "INNY GANG W STREFIE" };
                    default { "PRZEJMOWANIE" }
                };

				_ctrlStatusText ctrlSetStructuredText parseText format ["<t align='center' size='1.25'>%1</t>", _status];

				if !(_status isEqualTo "PRZEJMOWANIE") then {
	...
	default {
		...
		// Update Status Text...
		(_hud controlsGroupCtrl 103) ctrlSetStructuredText parseText format ["<t align='center' size='1.25'>%1</t>", (["ATAKOWANIE", "OBRONA"] select _isDefending)];
