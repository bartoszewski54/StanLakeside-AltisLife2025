...
["CartelPayout", {
	...
	if (_personal > 0) then {
		[_personal, true, format ["Wypłata z kartelu"]] call ULP_fnc_addMoney;
	};
	...
	if ([["NotifyPayment", "Cartels"] call ULP_fnc_getOption] call ULP_fnc_bool) then {
		[format ["Za okupowanie <t color='#B92DE0'>%1</t> otrzymujesz <t color='#B92DE0'>%2%3</t>, a Twoja grupa otrzymuje <t color='#B92DE0'>%2%4</t>", _cartel, "$", [_personal] call ULP_fnc_numberText, [_group] call ULP_fnc_numberText]] call fini_fnc_noty;
	};
