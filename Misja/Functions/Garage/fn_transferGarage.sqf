...
if !([getNumber (_missionCfg >> "canTransfer")] call ULP_fnc_bool) exitWith {
	["Ten pojazd nie może zostać przerejestrowany."] call fini_fnc_noty;
};
...
[ (findDisplay getNumber(configFile >> "RscDisplayMission" >> "idd")), [_faction], [_display, _id, _name, _classname, _faction], {
	...
	if !(isNil { player getVariable "vehicle_transfer" }) exitWith {
		["Oczekujesz obecnie na wynik innego przerejestrowania, poczekaj na jego zakończenie przed podjęciem kolejnej próby."] call fini_fnc_noty;
	};
	...
	["VehicleTransferred", {
		...
		private _message = _this param [0, "Pojazd został pomyślnie przerejestrowany."];
	...
	[format ["Żądanie przerejestrowania <t color='#B92DE0'>%1</t> na <t color='#B92DE0'>%2</t>...", _name, name _player]] call fini_fnc_noty;
