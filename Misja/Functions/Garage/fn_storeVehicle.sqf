...
if (isNil "_vehicle" || { isNull _vehicle } || { !(alive _vehicle) }) exitWith {
	["Brak twojego pojazdu w pobliżu!"] call fini_fnc_noty;
};

if ([[player] call ULP_fnc_getFaction, "vehicles"] call ULP_fnc_factionPresistant && { (_vehicle getVariable ["vehicle_id", -1]) >= 0  }) exitWith {
	[format["Chowanie %1...", ([typeOf _vehicle] call ULP_fnc_itemCfg) param [5, "Vehicle"]], 5, [_location, _vehicle], { (player distance (_this select 0)) <= 5 }, {
		_this params [ "", "_vehicle" ];

		["VehicleStored", { ["Pojazd schowany."] call fini_fnc_noty; }, true] call ULP_fnc_addEventHandler;
