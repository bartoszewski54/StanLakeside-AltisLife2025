...
class RscHUD {
    ...
    class ControlsBackground {
        class FoodBackground: Life_RscText {
            idc = -1;
            colorBackground[] = {0.1,0.1,0.1,0.85}; // Zielony
        ...
        class WaterBackground: Life_RscText {
            idc = -1;
            colorBackground[] = {0.1,0.1,0.1,0.85}; // Niebieski
            SAFEZONE_X((1 - WIDTH) - MARGIN_X);
        ...
        class HealthBackground: Life_RscText {
            idc = -1;
            colorBackground[] = {0.1,0.1,0.1,0.85}; // Czerwony
            SAFEZONE_X((1 - WIDTH) - MARGIN_X);
    ...
    class Controls {
        ...
        class HealthBar: Life_RscProgress {
            idc = 513;
            colorBar[] = {0.726, 0.233, 0.265, 0.85}; // Czerwony
            SAFEZONE_X((1 - WIDTH) - MARGIN_X);
			SAFEZONE_Y(((1 - BUTTON_H) - MARGIN_Y));
            SAFEZONE_W(WIDTH);
			SAFEZONE_H(BUTTON_H);
        ...
        class WaterBar: HealthBar {
            idc = 514;
			SAFEZONE_Y(((1 - (BUTTON_H * 2)) - MARGIN_Y) - (MARGIN_Y / 2));
            colorBar[] = {0, 0.325, 0.567, 0.85}; // Niebieski
        };

        class FoodBar: HealthBar {
            idc = 515;
			SAFEZONE_Y(((1 - (BUTTON_H * 3)) - MARGIN_Y) - MARGIN_Y);
            colorBar[] = {0.16, 0.561, 0.33, 0.85}; // Zielony
        };
