...
class Deconstruct : ULP_RscButtonClean {
	idc = 23015;
	text = "<t align = 'center'>Rozmontuj</t>";
	onButtonClick = " [""Rozmontowywanie jest obecnie niedostępne!""] call fini_fnc_noty; "
	//onButtonClick = "private _display = ctrlParent (_this select 0); private _list = _display displayCtrl 23016; if (isNull _display || { isNull _list }) exitWith {}; private _item = (_list lbData (lbCurSel _list)); if (_item isEqualTo """") exitWith { [""Musisz wybrać jakiś itemek!""] call fini_fnc_noty; }; [_item] call ULP_fnc_deconstruct;";
